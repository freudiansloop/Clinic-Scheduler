...
class AppUI(ctk.CTk):
    ...
    def setup_tab1(self):
        ...
        inst_frame = ctk.CTkFrame(self.tab_emp, height=50)
        inst_frame.pack(fill="x", padx=5, pady=5)
        
        # Using CTkTextbox for mixed colors (readonly)
        self.txt_inst = ctk.CTkTextbox(inst_frame, height=40, font=("Arial", 12), activate_scrollbars=False)
        self.txt_inst.pack(fill="both", expand=True, padx=2, pady=2)
        
        # Insert Text
        # "Active: include in schedule. Target: target number of shifts assigned. Clinic in 1st Half vs 2nd half of month (or no preference)."
        # "Preferred / Avoid dates. Date format examples: 12, 14AM, 15 PM, 3-17."
        # "Override force assigns those dates."
        
        text_content = (
            "Active: include in schedule. Target: target shifts. 1st Half / 2nd Half: clinic restriction.\n"
            "Preferred / Avoid: request dates (e.g. 12, 14AM, 15 PM, 3-17). Override: force assignment."
        )
        
        self.txt_inst.insert("0.0", text_content)
        
        # Apply Tags for Blue Color
        # We need to access the underlying Tkinter widget
        # Keywords: "Active:", "Target:", "1st Half", "2nd Half", "Preferred / Avoid", "Override"
        
        txt_widget = self.txt_inst._textbox
        txt_widget.tag_config("blue_tag", foreground="#87CEFA") # Light Blue
        
        keywords = ["Active:", "Target:", "1st Half", "2nd Half", "Preferred / Avoid", "Override", "Override:"]
        
        curr_text = txt_widget.get("1.0", "end")
        for kw in keywords:
            start_idx = "1.0"
            while True:
                pos = txt_widget.search(kw, start_idx, stopindex="end")
                if not pos: break
                end_pos = f"{pos}+{len(kw)}c"
                txt_widget.tag_add("blue_tag", pos, end_pos)
                start_idx = end_pos
        ...
        # Adjusted Column weights for visual alignment
        headers = ["Order", "Active", "Name", "Target", "1st Half", "2nd Half", "Preferred", "Avoid", "Override", "Color", "Del"]
        
        # Grid Configuration
        self.header_frame.grid_columnconfigure(0, weight=0, minsize=40) # Order
        self.header_frame.grid_columnconfigure(1, weight=0, minsize=40) # Active
        self.header_frame.grid_columnconfigure(2, weight=2) # Name
        self.header_frame.grid_columnconfigure(3, weight=0, minsize=50) # Target
        self.header_frame.grid_columnconfigure(4, weight=0, minsize=50) # 1st
        self.header_frame.grid_columnconfigure(5, weight=0, minsize=50) # 2nd
        self.header_frame.grid_columnconfigure(6, weight=3) # Pref
        self.header_frame.grid_columnconfigure(7, weight=3) # Avoid
        self.header_frame.grid_columnconfigure(8, weight=0, minsize=60) # Override
        self.header_frame.grid_columnconfigure(9, weight=0, minsize=40) # Color
        self.header_frame.grid_columnconfigure(10, weight=0, minsize=40) # Del
        ...
        # Match scroll columns to header
        self.p_scroll.grid_columnconfigure(0, weight=0, minsize=40)
        self.p_scroll.grid_columnconfigure(1, weight=0, minsize=40)
        self.p_scroll.grid_columnconfigure(2, weight=2)
        self.p_scroll.grid_columnconfigure(3, weight=0, minsize=50)
        self.p_scroll.grid_columnconfigure(4, weight=0, minsize=50)
        self.p_scroll.grid_columnconfigure(5, weight=0, minsize=50)
        self.p_scroll.grid_columnconfigure(6, weight=3)
        self.p_scroll.grid_columnconfigure(7, weight=3)
        self.p_scroll.grid_columnconfigure(8, weight=0, minsize=60)
        self.p_scroll.grid_columnconfigure(9, weight=0, minsize=40)
        self.p_scroll.grid_columnconfigure(10, weight=0, minsize=40)
    ...
    def generate_schedule(self):
        ...
        if val == "Automatic":
            # Rule: 16th is standard for 30/31 days. 
            # 28 days -> 15? Let's just stick to 16 as a safe default for now, 
            # OR simple logic: Halfway. 
            # 30 / 2 = 15. So 1-15 is 1st. 16+ is 2nd. 
            split = 16 
        else:
            try: split = int(val)
            except: split = 16
    ...
    def export_excel(self):
        ...
        try:
            ...
            # Borders: Default None, Medium for blocks
            # We will apply borders manually to specific cells
            border_medium_top = Border(top=Side(style='medium'))
            border_medium_bottom = Border(bottom=Side(style='medium'))
            border_medium_left = Border(left=Side(style='medium'))
            border_medium_right = Border(right=Side(style='medium'))
            
            fill_black = PatternFill("solid", fgColor="000000")
            
            # Map physician colors
            # Remove # from color string for openpyxl
            c_map = {p.name: p.color.replace("#","") for p in self.scheduler_logic.physicians}
            
            # --- LAYOUT SETUP ---
            # Column A is Buffer (Empty)
            ...
            # Column Widths (Tripled Black Bar, Reduced others by 20%)
            # Orig 25 -> 20
            col_width = 20
            gps = ['B', 'C', 'D', 'E', 'F']
            gps2 = ['H', 'I', 'J', 'K', 'L']
            ...
            ws.column_dimensions['G'].width = 12 # Tripled from 4
            ...
            for week in cal:
                ...
                # Draw Box Borders for the Week Row (per day)
                # We do this after filling content
                ...
                for day_idx in range(5):
                    ...
                    # Columns: Group A (B=2..F=6), Group B (H=8..L=12)
                    col_a = day_idx + 2
                    col_b = day_idx + 8
                    ...
                    am_docs = sched['AM']
                    if am_docs:
                        # Logic: If Mon(0)/Fri(4) -> Group B is Primary? 
                        # Wait, logic in V5 was: 
                        ...
                        if doc1 in c_map:
                             ws.cell(row=current_row+2, column=primary_col).fill = PatternFill("solid", fgColor=c_map[doc1])
                    ...
                    pm_docs = sched['PM']
                    if pm_docs:
                        # Logic: If Wed(2)/Thu(3) -> Group A is Primary
                        # Else (Mon/Tue/Fri) -> Group B? 
                        # V5 logic was: if day_idx == 2 or day_idx == 3: primary_group = 'A'
                        ...
                        if doc1 in c_map:
                             ws.cell(row=current_row+8, column=primary_col).fill = PatternFill("solid", fgColor=c_map[doc1])
                ...
                # --- BORDERS ---
                # "Make it default to no cell border. But use a medium thickness line for the borders of the calendar and the borders of each DAY."
                # We need to outline the BLOCK for each day.
                # Block A: (curr_row, col_a) to (curr_row+12, col_a)
                # Block B: (curr_row, col_b) to (curr_row+12, col_b)
                ...
                for day_idx in range(5):
                    ...
                    # Apply Medium Border to Outline of Day Box A
                    for r_off in range(13):
                        r = start_row_of_block + r_off
                        cell = ws.cell(row=r, column=col_a_idx)
                        
                        # Existing borders might be None.
                        # We only want outer edge medium. Inner items no border.
                    ...
                    # Apply Medium Border to Outline of Day Box B
                    for r_off in range(13):
                        r = start_row_of_block + r_off
            ...
            # --- SAVE CONFIG to SHEET 1 (A80) ---
            self.update_physician_objects_from_ui()
            data_list = [p.to_dict() for p in self.physicians]
            json_str = json.dumps(data_list)
            ws["A80"] = json_str
            ws["A80"].font = Font(color="FFFFFF") # Hide it white-on-white? Or just leave it.
            ...
            # HIDDEN SETTINGS in A80 of Sheet 2
            ws_cal["A80"] = json_str
